...
if hostname.endswith('hepnet.scphys.kyoto-u.ac.jp'): # @ Kyoto
    ...
    #devlocation = '/dev/ttyUSB0'
elif hostname == 'cmb-ocs' : # @ Kyoto
    #devlocation = '/dev/serial/by-id/usb-FTDI_FT232R_USB_UART_AB0KPFJJ-if00-port0'
    devlocation = '/dev/ttyUSB0'
else :
...
def setActuatorParameters(serial) :
    ...
    sendCommand(serial, '$111={}'.format(Fmax)) # speed [mm/min] Y-axis
    sendCommand(serial, '$112={}'.format(Fmax)) # speed [mm/min] Z-axis (not used)
    sendCommand(serial, '$120=10') # accel. [mm/sec^2] X-axis
    sendCommand(serial, '$121=10') # accel. [mm/sec^2] Y-axis
    sendCommand(serial, '$122=10') # accel. [mm/sec^2] Z-axis (not used)
    sendCommand(serial, '$130=1500') # max travel [mm] X-axis
    sendCommand(serial, '$131=1500') # max travel [mm] Y-axis
    sendCommand(serial, '$132=1500') # max travel [mm] Z-axis (not used)
    msg = 'Finished to set actuator controller parameters!'
    print(msg)
    return 0;
...
'''
printstatus(ser)
#sendCommand(ser, 'G91 F1000 X+10', verbose=1)
printstatus(ser)
#sendCommand(ser, 'G91 G1 F1000 Y+10', verbose=1)
printstatus(ser)

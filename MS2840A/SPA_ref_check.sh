...
LOG="/data/ms2840a/SPA_ref_check/SPA_ref_check.log"


echo ref mode: ON / OFF / TEST / etc ?
read MODE
echo number?
read Num
echo -n "SPA_ref_check/try_$Num/ref_$MODE, " >> $LOG

# 探索データをここに保存する
SEARCH="/data/ms2840a/SPA_ref_check/try_$Num/ref_$MODE/"
...
for ((TIMES=1; TIMES<=$((repeating_time)); TIMES++)); do
    ...
    SEARCH_i_times="/data/ms2840a/SPA_ref_check/try_$Num/ref_$MODE/set_$TIMES"
    mkdir ${SEARCH_i_times}

    command="python3 SPA_ref_check.py -m SEARCH --fstart $FREQ -o $SEARCH_i_times $OPT"
    echo $command

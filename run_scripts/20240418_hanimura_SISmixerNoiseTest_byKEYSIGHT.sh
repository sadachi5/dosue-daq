...
echo "finishing LO[GHz] = ?"
read LO_f

echo "OK, you can measure from $((LO)) to $((LO_f))."
...
repeating_time=$((LO_f+1-LO))

for ((i=1; i<$((repeating_time+1)); i++)); do

echo "Keysignt ON! $((LO+i-1)) GHz (˙꒳​˙ᐢ)push!"
python ../E8257D/E8257D.py --on -f $(((LO+i-1)*1000000000/18)) -p 9.5
...
if [ $YN = "y" ]; then
    ...
    python3 ../MS2840A/MS2840A.py -f DOSUE-Y_test_SWEEP_NoiseTest_Keysight_LO$((LO+i-1))_300K_tuned -m 'SWEEP' -s $start -w $width -r $rbw -n ${nLoop} --att 0 --nRun ${nRun} --overwrite
...
if [ $YN2 = "y" ]; then
    ...
    python3 ../MS2840A/MS2840A.py -f DOSUE-Y_test_SWEEP_NoiseTest_Keysight_LO$((LO+i-1))_77K_tuned -m 'SWEEP' -s $start -w $width -r $rbw -n ${nLoop} --att 0 --nRun ${nRun} --overwrite
